...
def convert_tensor(key, d):
    ...
    c = torch.from_numpy(np.array(d[key], np.float32, copy=False)).transpose(0, 1).contiguous().view(1, d[key].size[0],
                                                                                                     d[key].size[1])
    d = (255.0 - c) / 255.0
    return d
...
class FEMNIST(data.Dataset):
    ...
    def generate_ds(self, args, root):
        ...
        num_img = args.train_shots_max * args.all_num_users
        ...
        for i in range(num_class):
            for k in range(num_img):
                img = os.path.join(root, 'data', 'raw_data', 'by_class', files[i], 'train_' + files[i],
    ...
    def generate_ds_test(self, args, root):
        ...
        for i in range(num_class):
            for k in range(num_img):
                img = os.path.join(root, 'data', 'raw_data', 'by_class', files[i], 'hsf_0',
