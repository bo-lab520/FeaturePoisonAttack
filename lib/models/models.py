...
class KnowledgeExtractModule(nn.Module):
    def __init__(self):
        self.conv1 = nn.Conv2d(1, 10, kernel_size=3)
        self.conv2 = nn.Conv2d(10, 10, kernel_size=3)
        self.fc1 = nn.Linear(int(320 / 20 * 10), 50)
...
class CNNMnist(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(F.max_pool2d(self.conv2_drop(self.conv2(x)), 2))
        x = x.view(-1, x.shape[1]*x.shape[2]*x.shape[3])
        # 原型, 经过模型前几层计算后的数据,作为当前特征数据x的原型
        x1 = F.relu(self.fc1(x))
        x = F.dropout(x1, training=self.training)
        x = self.fc2(x)
        return x, F.log_softmax(x, dim=1), x1  # 返回标签和原型
...
class CNNMnist_client(nn.Module):
    def __init__(self, args):
        super(CNNMnist, self).__init__()
        self.conv1 = nn.Conv2d(args.num_channels, 10, kernel_size=5)
        self.conv2 = nn.Conv2d(10, args.out_channels, kernel_size=5)
...
class CNNMnist_server(nn.Module):
    def __init__(self, args):
        super(CNNMnist, self).__init__()
        self.fc2 = nn.Linear(50, args.num_classes)
