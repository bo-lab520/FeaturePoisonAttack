    ...
    if args.dataset == 'mnist':
        ...
        # 获得各个客户端非独立同分布数据
            ...
            if args.unequal:
                ...
                user_groups = mnist_noniid_unequal(args, train_dataset, args.all_num_users)
            else:
                # Chose euqal splits for every user
                # 为每一个客户端获取非独立同分布训练数据
                user_groups, classes_list = mnist_noniid(args, train_dataset, args.all_num_users, n_list, k_list)
                # 为每一个客户端获取非独立同分布测试数据
                user_groups_lt = mnist_noniid_lt(args, test_dataset, args.all_num_users, n_list, k_list, classes_list)
        ...
        if args.iid:
            ...
            if args.unequal:
                ...
                # Chose euqal splits for every user
                user_groups, classes_list, classes_list_gt = cifar10_noniid(args, train_dataset, args.all_num_users, n_list,
                                                                            k_list)
                user_groups_lt = cifar10_noniid_lt(args, test_dataset, args.all_num_users, n_list, k_list, classes_list)
        ...
        if args.iid:
            ...
            if args.unequal:
                ...
                # Chose euqal splits for every user
                user_groups, classes_list, classes_list_gt = cifar100_noniid(args, train_dataset, args.all_num_users, n_list, k_list)
                user_groups_lt = cifar100_noniid_lt(test_dataset, args.all_num_users, classes_list)
...
def label_scrambling(_labels):
    ...
    for index in range(length):
        ...
        # c_labels[index] = (_labels[index]+1) % 10
    return c_labels
